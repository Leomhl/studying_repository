	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			case "color":
				...
				$output .='<button id="' . esc_attr( $value['id'] ) . '" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" class="btn btn-default" data-iconset="fontawesome" data-icon="' . esc_attr( $val ) . '" role="iconpicker" value="' . esc_attr( $val ) . '"></button>';
